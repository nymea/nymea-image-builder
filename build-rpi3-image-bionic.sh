...
SCRIPTDIR=$(pwd)
BASEDIR=${SCRIPTDIR}/image-rpi3-build-bionic
...
function bootstrap() {
    ...
    if [ ! -f "${R}/tmp/.bootstrap" ]; then
        if [ "${ARCH}" == "armv7l" ]; then
            ...
            qemu-debootstrap --verbose --foreign --arch=armhf $RELEASE $R http://ports.ubuntu.com/
        fi
...
## guh repo
deb http://repository.nymea.io ${RELEASE} main raspberry
deb-src http://repository.nymea.io ${RELEASE} main raspberry
EOM
...
#    chroot $R apt-get -y install libraspberrypi-bin libraspberrypi-dev libraspberrypi-doc libraspberrypi0 raspberrypi-bootloader rpi-update raspi-config \
#                                  bluez-firmware linux-firmware pi-bluetooth fake-hwclock fbset i2c-tools rng-tools raspi-gpio raspberrypi-sys-mods 
